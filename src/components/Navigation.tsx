      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-8">
            <Link href={isAdmin ? '/admin' : '/courses'} className="flex items-center gap-2 shrink-0">
              <div className="w-7 h-7 bg-blue-500 rounded-lg flex items-center justify-center">
                <Sparkles className="w-4 h-4 text-white" />
              </div>
              <span className="font-bold text-gray-900 text-[15px] tracking-tight hidden sm:block">
                ENISHI <span className="text-blue-500">LESSONS</span>
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-0.5">
              {isAdmin ? (
                  ...
                  return (
                    <Link key={href} href={href}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium transition-colors ${
                        active ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'
                      }`}>
                ...
                <Link href="/courses"
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium transition-colors ${
                    pathname.startsWith('/courses') ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'
                  }`}>
          ...
          <div className="flex items-center gap-2">
            {isAdmin && (
              <span className="hidden sm:block text-[11px] bg-blue-50 text-blue-600 border border-blue-200 px-2.5 py-1 rounded-full font-semibold">
                管理者
              </span>
            )}
            <div className="relative">
              <button onClick={() => setMenuOpen(!menuOpen)}
                ...
                <span className="text-sm text-gray-700 hidden sm:block max-w-[120px] truncate">
                  {(user as { displayName?: string }).displayName || user.email.split('@')[0]}
                </span>
              ...
              {menuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1.5 w-52 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-20 overflow-hidden">
