    ...
    <div className="px-4 sm:px-6 py-8 max-w-2xl mx-auto">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className={labelCls}>コース名 <span className="text-rose-500 normal-case">*</span></label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required
              className={inputCls} placeholder="例：Pythonプログラミング入門" />
          </div>
          <div>
            <label className={labelCls}>説明 <span className="text-rose-500 normal-case">*</span></label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} required rows={4}
              className={`${inputCls} resize-none`}
              placeholder="このコースで学べること、対象者、ゴールなどを説明してください" />
          </div>
          <div>
            <label className={labelCls}>サムネイルURL <span className="text-zinc-600 normal-case font-normal">(任意)</span></label>
