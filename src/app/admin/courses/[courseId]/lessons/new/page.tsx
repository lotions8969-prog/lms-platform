          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required
                className={inputCls} placeholder="例：第1回 はじめてのレッスン" />
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3 flex-wrap">
                  {([
                    ...
                    { mode: 'library', icon: Library, label: 'ライブラリから選択' },
                    { mode: 'url', icon: Link2, label: 'URLを入力' },
                ...
                {videoInputMode === 'upload' && (
                  <div>
                    {uploadError && (
                      <div className="flex items-start gap-2 bg-rose-950/40 border border-rose-900/50 text-rose-400 text-sm px-3 py-2 rounded-xl mb-3">
                      ...
                      <div onClick={() => !uploading && fileInputRef.current?.click()}
                        ...
                        {uploading ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-zinc-700 mt-1">MP4, MOV, AVI, WebM など対応（最大500MB）</p>
                          </>
                        )}
                      </div>
                    ) : (
                  ...
                  <div>
                    {selectedLibraryVideo ? (
                      ...
                      <div className="text-center py-12 border-2 border-dashed border-zinc-800 rounded-xl text-zinc-600">
                        ...
                        <Link href="/admin/videos" target="_blank" className="text-xs text-violet-400 hover:underline mt-1 inline-block transition-colors">
                          動画ライブラリでアップロードする →
                        </Link>
                      </div>
                    ) : (
                          ...
                          {libraryVideos.map((v) => (
                            <button key={v.url} type="button" onClick={() => setSelectedLibraryVideo(v)}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-zinc-600">{formatBytes(v.size)} · {new Date(v.uploadedAt).toLocaleDateString('ja-JP')}</p>
                              </div>
                              <button type="button" onClick={(e) => { e.stopPropagation(); setPreviewVideo(v); }}
                                className="p-1.5 text-zinc-600 hover:text-violet-400 hover:bg-violet-900/20 rounded-lg transition-colors shrink-0">
                ...
                {videoInputMode === 'url' && (
                  <div>
                    ...
                    <p className="text-xs text-zinc-600 mt-1.5">MP4/WebM ファイルのURL、またはYouTube埋め込みURLを入力</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setPreviewVideo(null)}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-zinc-800">
              ...
              <button onClick={() => setPreviewVideo(null)} className="text-zinc-500 hover:text-zinc-300 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-4 flex justify-end gap-3">
              <button onClick={() => setPreviewVideo(null)}
                className="px-4 py-2 text-sm border border-zinc-700 text-zinc-400 rounded-xl hover:bg-zinc-800 transition-colors">閉じる</button>
              <button onClick={() => { setSelectedLibraryVideo(previewVideo); setPreviewVideo(null); }}
                className="px-4 py-2 text-sm bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-semibold transition-all">
                この動画を使用する
              </button>
            </div>
          </div>
