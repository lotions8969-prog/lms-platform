    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href={`/admin/courses/${courseId}/lessons/new`} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400"><p>レッスンがまだありません</p></div>
        ) : (
            ...
            {lessons.map((lesson) => (
              <div key={lesson.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                ...
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${lesson.type === 'quiz' ? 'bg-purple-100' : 'bg-blue-100'}`}>
                  {lesson.type === 'quiz' ? <FileQuestion className="w-5 h-5 text-purple-600" /> : <Play className="w-5 h-5 text-blue-600" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-900 truncate">{lesson.title}</p>
                  <p className="text-xs text-gray-400">順番: {lesson.order} · {lesson.type === 'quiz' ? 'クイズ' : '動画'}</p>
                </div>
                <button onClick={() => handleDelete(lesson.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        )}
