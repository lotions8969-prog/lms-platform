...
export default function CoursesPage() {
  ...
  const isCompleted = (courseId: string) => userData?.progress?.[courseId]?.completed;
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => {
              ...
              const progress = userData?.progress?.[course.id];
                ...
                <Link key={course.id} href={`/courses/${course.id}`} className="group">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all hover:-translate-y-0.5">
                    ...
                    <div className="p-5">
                      ...
                      {completedCount > 0 && (
                        <p className="text-xs text-blue-600 mt-2 font-medium">{completedCount}レッスン完了</p>
                      )}
                      <div className="flex items-center justify-between mt-4">
                        <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${completed ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                          {completed ? '完了' : '学習中'}
                        </span>
