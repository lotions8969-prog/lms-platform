      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="space-y-3">
          {lessons.map((lesson, index) => {
            ...
            if (isLocked) return (
              <div key={lesson.id} className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-200 opacity-60 cursor-not-allowed">
                <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center shrink-0">
                  <Lock className="w-5 h-5 text-gray-400" />
                </div>
                <div className="flex-1"><p className="font-medium text-gray-700">{lesson.title}</p><p className="text-xs text-gray-400 mt-0.5">前のレッスンを完了してください</p></div>
              </div>
            );
            return (
              <Link key={lesson.id} href={`/courses/${courseId}/lessons/${lesson.id}`}>
                <div className={`flex items-center gap-4 p-4 bg-white rounded-xl border transition-all hover:shadow-md hover:-translate-y-0.5 ${done ? 'border-green-200' : 'border-gray-200'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${done ? 'bg-green-100' : 'bg-blue-100'}`}>
                    {done ? <CheckCircle className="w-5 h-5 text-green-600" /> : lesson.type === 'quiz' ? <FileQuestion className="w-5 h-5 text-blue-600" /> : <Play className="w-5 h-5 text-blue-600" />}
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-gray-900">{lesson.title}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{done ? '完了' : lesson.type === 'quiz' ? 'クイズ' : '動画レッスン'}</p>
                  </div>
